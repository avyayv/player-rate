    ...
    if found_id:
        ...
        try:
            ...
            if get_ids.define_player.players.index(player) != len(get_ids.define_player.players)-1:
                print get_ids.define_player.players[get_ids.define_player.players.index(player)].id, player.name, str((float(number)/float(len(get_ids.define_player.players)))*100)+"%"
            else:
                print "Its ZUBAC"
            with open('data.json', 'a') as file:
