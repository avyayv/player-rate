...
parser.add_argument('startid', metavar='lastid', type=str, nargs='+',
...
for year in years:
    ...
    for player in defData["resultSets"][0]["rowSet"]:
        advanced = get_ids.define_player.PlayerTracking(year, player[11], player[12], player[13], "", "", "",
        "", "", "", "", "", "",
        "", "", "", "", "", "",
        "", "", "", "", "")
        get_ids.define_player.findById(player[0]).advanced_statistics.append(advanced)
    for player in driveData["resultSets"][0]["rowSet"]:
        advanced = get_ids.define_player.findById(player[0]).find_advanced_for_year(year)
