...
valid_player = False
data = json.load(open("newer_data.json"))
...
for i in range(1,30):
    ...
    if poa.lower() == "pts":
        playery.append(loaded_modeltwo.predict(np.array(an))[-1][0]*50)
        plt.xlabel(player_name+' Minutes', fontsize=15)
        plt.ylabel(player_name+' Points', fontsize=15)
    elif poa.lower() == "reb":
        print(loaded_modelthree.predict(np.array(an))[-1][0]*50)
        playery.append(loaded_modelthree.predict(np.array(an))[-1][0]*50)
        plt.xlabel(player_name+' Minutes', fontsize=15)
        plt.ylabel(player_name+' Rebounds', fontsize=15)
    elif poa.lower() == "blk":
        print(loaded_modelfour.predict(np.array(an))[-1][0]*50)
        playery.append(loaded_modelfour.predict(np.array(an))[-1][0]*50)
        plt.xlabel(player_name+' Minutes', fontsize=15)
        plt.ylabel(player_name+' Blocks', fontsize=15)
    elif poa.lower() == "stl":
        print(loaded_modelfive.predict(np.array(an))[-1][0]*50)
        playery.append(loaded_modelfive.predict(np.array(an))[-1][0]*50)
        plt.xlabel(player_name+' Minutes', fontsize=15)
        plt.ylabel(player_name+' Steals', fontsize=15)
    elif poa.lower() == "win":
        print(loaded_modelsix.predict(np.array(an))[-1][0])
        playery.append(loaded_modelsix.predict(np.array(an))[-1][0])
        plt.xlabel(player_name+' Minutes', fontsize=15)
        plt.ylabel(player_name+' Wins', fontsize=15)
    else:
        print(loaded_modelone.predict(np.array(an))[-1][0]*50)
        playery.append(loaded_modelone.predict(np.array(an))[-1][0]*50)
        plt.xlabel(player_name+' Minutes', fontsize=15)
